...
class ProductionWikipediaDataPipeline:
    def __init__(self, config_path: str = "config/config.yaml"):
        """Initialize the production Wikipedia data pipeline."""
        ...
        self.mediawiki_client = ProductionMediaWikiClient(
        ...
        self.data_processor = ProductionDataProcessor()
    ...
    def initialize(self) -> bool:
        ...
        try:
            self.logger.info("Initializing Production Wikipedia Data Pipeline...")
            
            # Test MediaWiki API connection using mwclient
            self.logger.info("Testing MediaWiki API connection with mwclient...")
            if not self.mediawiki_client.test_connection():
                self.logger.error("Failed to connect to MediaWiki API")
                return False
            
            # Get site information
            site_info = self.mediawiki_client.get_site_info()
            self.logger.info(f"Connected to: {site_info.get('sitename', 'Unknown')}")
            ...
            libraries = self.mediawiki_client.get_available_libraries()
            self.logger.info(f"Available libraries: {libraries}")
            
            if not libraries.get('mwclient', False):
                self.logger.error("mwclient library is required for production environment")
                return False
    ...
    def populate_queue(self, limit: Optional[int] = None) -> int:
        """Populate the queue with Wikipedia articles using mwclient."""
        try:
            self.logger.info("Fetching Wikipedia articles with mwclient...")
            ...
            # Get all articles from MediaWiki API using mwclient
    ...
    def process_articles(self) -> Dict[str, int]:
        """Process articles from the queue using mwclient + wikitextparser."""
        try:
            self.logger.info("Starting article processing with mwclient + wikitextparser...")
            ...
            while not self.queue_manager.is_empty():
                ...
                try:
                    # Fetch article content using mwclient
                    title = article.get('title', '')
                    self.logger.debug(f"Processing article: {title}")
                    
                    article_content = self.mediawiki_client.get_article_content(title)
                    
                    if article_content:
                        # Process article data with production processor
                        ...
                        if self.data_processor.validate_article(processed_article):
                            ...
                            source = processed_article.get('source', 'unknown')
                            self.logger.debug(f"Article {title} processed with source: {source}")
                        else:
    ...
    def _upload_batch(self, articles: List[Dict[str, Any]], batch_number: int, start_index: int):
        ...
        try:
            ...
            if success:
                ...
                summary = self.data_processor.create_summary(articles)
                self.logger.info(f"Batch {batch_number} summary: {summary}")
            else:
    ...
    def run(self, limit: Optional[int] = None) -> Dict[str, Any]:
        """Run the complete production Wikipedia data pipeline."""
        try:
            self.logger.info("Starting Production Wikipedia Data Pipeline")
            ...
            self.logger.info(f"Production pipeline completed successfully: {summary}")
            return summary
...
def main():
    """Main entry point for the production Wikipedia data pipeline."""
    import argparse
    
    parser = argparse.ArgumentParser(description='Production Wikipedia Data Pipeline')
    ...
    pipeline = ProductionWikipediaDataPipeline(args.config)
